...
PostgreSQL. 4 main tables.

## Tables

### users

User profile with data for calculations (protein score, TDEE).
...
CREATE TABLE fasts (
  ...
  end_time TIMESTAMPTZ,                    -- NULL = in progress
...
Each logged meal. The `source` field indicates how macros were entered.
...
One row per day per user. Upsert on (user_id, date).
...
users 1──N fasts
users 1──N meals
users 1──N daily_metrics (max 1 per day)
```
